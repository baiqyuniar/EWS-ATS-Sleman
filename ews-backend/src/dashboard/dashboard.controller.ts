...
export class DashboardController {
  ...
  async getTopRiskStudents(@CurrentUser() user: CurrentUserPayload) {
    if (user.role === 'SEKOLAH' && !user.schoolId) {
      throw new ForbiddenException('Akun sekolah tidak terhubung ke sekolah manapun');
    }
    return this.service.getTopRiskStudents(user.role === 'SEKOLAH' ? user.schoolId : undefined);
  }
  ...
  async schoolAnalytics(@Param('schoolId') schoolId: string, @CurrentUser() user: CurrentUserPayload) {
    const id = Number(schoolId);
    if (user.role === 'SEKOLAH' && user.schoolId !== id) {
      throw new ForbiddenException('Tidak berwenang mengakses dashboard sekolah lain');
    }
  ...
  async kapanewonAnalytics(@Param('kapanewon') kapanewon: string, @CurrentUser() user: CurrentUserPayload) {
