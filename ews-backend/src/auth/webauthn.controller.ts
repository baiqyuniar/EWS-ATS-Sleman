...
export class WebauthnController {
  ...
  async loginVerify(
    ...
    const session = await this.authService.issueSession(result.user);
    // SECURITY: sama seperti login password (lihat AuthController.login) —
    // token disimpan di cookie httpOnly, bukan di body JSON.
    res.cookie(ACCESS_TOKEN_COOKIE, session.accessToken, getAccessTokenCookieOptions(this.config));
    return { verified: true, user: session.user };
  }
}