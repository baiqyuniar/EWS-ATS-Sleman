...
export const getMonthlyTrend = async (): Promise<MonthlyTrendItem[]> => {
  const { data } = await api.get<MonthlyTrendItem[]>(
...
export const getTopRiskStudents = async (): Promise<TopRiskStudent[]> => {
  const { data } = await api.get<TopRiskStudent[]>(
    "/dashboard/top-risk-students",
  );
  return data;
};
...
export const getSchoolRiskTrend = async (
  ...
  const { data } = await api.get<SchoolRiskTrendItem[]>(
    `/dashboard/schools/${schoolId}/risk-trend`,
    {
      params: { period },
    },
  );
...
export const getKapanewonHeatmap = async (
  mode: "residence" | "school" = "residence",
): Promise<KapanewonHeatmapItem[]> => {
  const { data } = await api.get<KapanewonHeatmapItem[]>(
    "/dashboard/kapanewon-heatmap",
    {
      params: { mode },
    },
  );
...
export const getSchoolAnalytics = async (
  ...
  const { data } = await api.get<SchoolAnalytics>(
    `/dashboard/schools/${schoolId}/analytics`,
  );
  return data;
};

export const getKapanewonAnalytics = async (
  kapanewon: string,
): Promise<KapanewonAnalytics> => {
